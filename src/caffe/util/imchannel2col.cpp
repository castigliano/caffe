	...
	for (int c = 0; c < kernel_sz; ++c) {
		...
		for (int h = 0; h < height_col; ++h) {
			for (int w = 0; w < width_col; ++w) {
				...
				h_pad = caffe_cpu_modulus_temp(h_pad, chHeight);
				w_pad = caffe_cpu_modulus_temp(w_pad, chWidth);
				//
...
void colchannel2im_cpu(const Dtype* data_col, const int channels,
	...
	caffe_set(imHeight * imWidth * channels, Dtype(0), data_im);
...
// Explicit instantiation
template void colchannel2im_cpu<float>(const float* data_col, 
		const int channels,
    const int imHeight, const int imWidth, 
    const int chHeight, const int chWidth, 
		const int patch_h, const int patch_w,
...
template void colchannel2im_cpu<double>(const double* data_col,
		const int channels,
    const int imHeight, const int imWidth,
    const int chHeight, const int chWidth,
	  const int patch_h, const int patch_w,
